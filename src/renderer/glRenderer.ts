...
void main() {
  ...
  const updateStandardUniforms = (prog: WebGLProgram, state: RenderState) => {
    ...
    // Performance-safe feature toggling
    const getSafeEnabled = (enabled: boolean, cost: 'low' | 'medium' | 'high' = 'low') => {
      if (!enabled) return 0;
      if (state.sessionHealth?.status === 'critical') return 0;
      // In performance mode, we drop 'high' cost features if we are even slightly degraded
      if (state.performanceMode && state.sessionHealth?.status === 'degraded' && cost === 'high') return 0;
      return 1;
    };
    ...
    gl.uniform1f(getLocation('uPlasmaEnabled'), getSafeEnabled(state.plasmaEnabled, 'low'));
    ...
    gl.uniform1f(getLocation('uGlyphEnabled'), getSafeEnabled(state.glyphEnabled, 'medium'));
    ...
    gl.uniform1f(getLocation('uCrystalEnabled'), getSafeEnabled(state.crystalEnabled, 'medium'));
    ...
    gl.uniform1f(getLocation('uInkEnabled'), getSafeEnabled(state.inkEnabled, 'high'));
    ...
    gl.uniform1f(getLocation('uTopoEnabled'), getSafeEnabled(state.topoEnabled, 'high'));
    ...
    gl.uniform1f(getLocation('uWeatherEnabled'), getSafeEnabled(state.weatherEnabled, 'medium'));
    ...
    gl.uniform1f(getLocation('uPortalEnabled'), getSafeEnabled(state.portalEnabled, 'medium'));
    ...
    gl.uniform1f(getLocation('uMediaEnabled'), getSafeEnabled(state.mediaEnabled, 'low'));
    ...
    gl.uniform1f(getLocation('uEffectsEnabled'), getSafeEnabled(state.effectsEnabled, 'medium'));
    ...
    gl.uniform1f(getLocation('uParticlesEnabled'), getSafeEnabled(state.particlesEnabled, 'high'));
    ...
    gl.uniform1f(getLocation('uSdfEnabled'), getSafeEnabled(state.sdfEnabled, 'medium'));
