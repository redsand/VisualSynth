...
import { buildFragmentShader, shaderCacheKey } from './render/shaderBuilder';
...
void main() {
  ...
  // Shader preamble containing version, precision, non-generator uniforms, and utility functions
  const SHADER_PREAMBLE = `#version 300 es
...
`;
  ...
  const recompileForGenerators = (activeIds: Set<string>): boolean => {
    // Store the active IDs for future recompilations (e.g., when SDF changes)
    currentActiveIds = new Set(activeIds);

    const t0 = performance.now();
    const wasCached = programCache.has(shaderCacheKey(activeIds, currentSdfMapBody, currentPlasmaSource));
  ...
  const precompileVariant = (ids: Set<string>): void => {
    ...
    const t0 = performance.now();
    const prog = getOrCompileProgram(ids, currentSdfUniforms, currentSdfFunctions, currentSdfMapBody, currentPlasmaSource);
    const elapsed = (performance.now() - t0).toFixed(1);
    const cached = !prog || elapsed === '0.0';
    console.log(`[Shader] Precompiled ${ids.size} generators in ${elapsed}ms (cached: ${cached})`);
  };

  return { render, setLayerAsset, setPalette, setPlasmaShaderSource, getLastShaderError, getGeneratorDiagnostics, getMissingUniforms, recompileForGenerators, precompileVariant };
};
