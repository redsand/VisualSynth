...
## Storage
- Presets: `assets/presets/*.json` (currently v3, layer-only format)
...
4. **Main process** loads preset JSON  
   ...
   - Validates and returns project payload
...
- V3 preset is converted to v2 project via `applyPresetV3()`
- `applyProject()` updates the active scene/layers
- Static configuration (effects/particles/sdf/style/etc.) is sourced from the current project

## Render Consumption
- `src/renderer/index.ts` render loop extracts layer + FX state  
  - Layer IDs: `layer-plasma`, `layer-spectrum`, etc.
  - Layer params now respected for speed/scale/elevation (post-fix)
