...
- Macro controls first, details later: macro hero controls lead, advanced panels follow.
- Visual feedback on every interaction: focus/active states, interaction flashes, and live canvas response.

## Layout Overview

1) Scene Strip (Top)
- Always visible across all modes.
- Scene selection, quantize controls, and quick scene actions are surfaced above everything else.
...
3) Scene Macros (Right, Always Visible)
- Four high-signal macros: Energy, Motion, Color, Density.
- These map to Macro 1–4 for performance-first control.
- Values update with the active scene to preserve performance continuity.
...
- Macro hero keeps the “flow layer” within thumb reach and supports one-screen launch.
- Preset Explorer lives under Performance for deeper browsing without crowding the scene strip.
